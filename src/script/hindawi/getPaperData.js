...
async function getPaperData(url) {
  ...
  return Array.from(elements).map((value) => {
    return {
      id: parseInt(
        value
          .querySelector("ul.toc_article__header li:nth-child(4)")
          .textContent.match(/(\d+)/g)
      ),
      journal: value
        .querySelector("ul.toc_article__header li")
        .textContent.replace(" -", "")
        .trim(),
      volume: parseInt(
        value.querySelector("ul li:nth-child(3)").textContent.match(/(\d+)/g)
      ),
      link: "https://www.hindawi.com" + value.querySelector("a").href,
      title: value.querySelector("h2").textContent,
      publishedTime: value.querySelector("ul.toc_article__footer li")
        .textContent,
...
async function start() {
  ...
  await Promise.all(
    journalList.map((journal, jindex) => {
      return limitJournal(async () => {
        ...
        for (let index = 1; index <= totalPage; index++) {
          queue.push(
            limitPage(async () => {
              ...
              let result = await getPaperData(
                `${journal.link}contents/page/${index}/`
              );
        ...
        await writeFile(
          `../resources/paperdata/journal${jindex + 1}.json`,
